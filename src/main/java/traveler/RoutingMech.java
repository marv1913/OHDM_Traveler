...
public class RoutingMech {
	...
	private void createBasicRoadMap_basedOnTM(SqlStatement sql){
		...
		List<String> roadMapForTM = TransportMittel.getAccessWaysForTransportType(searchParameter.getTransportType());
	...
	private void createAndFillTimeTable(SqlStatement sql){
		...
		List<String> waysSubClasses_TM = TransportMittel.getAccessWaysForTransportType(searchParameter.getTransportType());
		List<String> waysSubClasses_Water = TransportMittel.getAccessWaysForTransportType("5");
	...
	private String countFullTime(SqlStatement sql, Float distanceLand, Float distanceWater){
		try {
			...
			sql.append("(SELECT speed_1, speed as speed_2  FROM (SELECT speed as speed_1 FROM "
					+ TransportMittel.getTransportMittelTable()
					+ " WHERE id = "
					+ searchParameter.getTransportType()
					+ ") as tm_1");
